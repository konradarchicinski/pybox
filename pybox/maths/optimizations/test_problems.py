...
def eggholder_function(x):
    ...
    return (-(x[1] + 47) * np.sin(np.sqrt(np.absolute(x[1] + x[0] / 2 + 47))) -
...
def goldstein_price_function(x):
    ...
    return ((1 + (19 - 14 * x[0] + 3 * x[0]**2 - 14 * x[1] + 6 * x[0] * x[1] +
                  3 * x[1]**2) * (x[0] + x[1] + 1)**2) *
            (30 + (18 - 32 * x[0] + 12 * x[0]**2 + 48 * x[1] -
                   36 * x[0] * x[1] + 27 * x[1]**2) * (2 * x[0] - 3 * x[1])**2))
...
def langermann_function_values(domain=None,
                               samples=300,
                               array_a=None,
                               array_c=None):
    if array_a is None:
